...
class BudgetEntity(id: EntityID<Int>) : IntEntity(id) {
    ...
    fun toResponse(): BudgetRecord {
        return BudgetRecord(year, month, amount, type, null)
    }

    fun toResponseBudgetAuthor(): BudgetRecordAllInfo {
        return BudgetRecordAllInfo(
            year = year,
            month = month,
            amount = amount,
            type = type,
            authorName = author?.name,
            authorCreation = author?.creation?.toString("yyyy-MM-dd HH:mm:ss")
        )
    }
}